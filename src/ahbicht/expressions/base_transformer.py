...
SupportedArgumentNodeType = TypeVar("SupportedArgumentNodeType")  # bound=ConditionNode)
...
class BaseTransformer(Transformer, ABC, Generic[SupportedArgumentNodeType, TSupportedReturnType]):
    ...
    def __init__(self, input_values: Mapping[str, SupportedArgumentNodeType]):
        """
        The input are the evaluated format constraint conditions in the form of ConditionNodes.
    ...
    def condition(self, token: Token) -> SupportedArgumentNodeType:
    ...
    def package(self, token: Token) -> SupportedArgumentNodeType:
