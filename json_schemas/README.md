...
There is a `uv` dependency group that provides everything you need to run [`generate_json_schemas.py`](generate_json_schemas.py) and overwrite any `.json` files in this directory.
Simply run:

```bash
uv run --group json_schemas python json_schemas/generate_json_schemas.py
```
